...
// 「读回自己的标记」这条判据也就没有意义了。
module mkFast(RegIf#(AW, DW));
  Vector#(4, Reg#(Bit#(DW))) v <- replicateM(mkReg('hDEAD0000));
...
module mkSlow#(Integer n)(RegTarget#(AW, DW));
  ...
  Vector#(4, Array#(Reg#(Bit#(DW)))) v <- replicateM(mkCReg(2, 'hBEEF0000));
  ...
  method RegRsp#(DW) rsp = RegRsp { rdata: v[ix[1]][1], err: False };
endmodule
...
// 方法每拍都在，规则就永不触发。
interface FakeMgr;
  interface RegManager#(AW, DW) m;
  method Bool fin;
  method Bool bad;
endinterface

module mkProg#(Bit#(AW) addr, Bit#(DW) tag, Bit#(16) rounds,
               Bit#(8) gap)(FakeMgr);
  Reg#(Bool)           v     <- mkReg(False);
  Reg#(RegReq#(AW, DW)) rq    <- mkReg(unpack(0));
  ...
  Wire#(RegRsp#(DW))   rx    <- mkDWire(unpack(0));

  function RegReq#(AW, DW) mk(Bool w) =
      RegReq { addr: addr, write: w, wdata: tag, wstrb: w ? '1 : 0 };
  ...
  interface RegManager m;
    method Bool valid = v;
    method RegReq#(AW, DW) req = rq;
    method Action ready(Bool r);
    endmethod
    method Action resp(Bool y, RegRsp#(DW) x);
      rv <= y;
      rx <= x;
    endmethod
  endinterface

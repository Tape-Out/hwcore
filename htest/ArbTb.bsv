...
module mkArbTb(Empty);
  ...
  FakeMgr p0 <- mkProg('h04, 'hA1A1A1A1, 60, 17);   // 零等待那一路
  FakeMgr p1 <- mkProg('h24, 'hB2B2B2B2, 60, 0);    // 会停顿那一路，一刻不停
  FakeMgr p2 <- mkProg('h28, 'hC3C3C3C3, 60, 11);   // 同一个慢设备，另一个地址

  Vector#(3, RegManager#(AW, DW)) ms = cons(p0.m, cons(p1.m, cons(p2.m, nil)));
